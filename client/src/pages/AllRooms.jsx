...
const RadioButton=({lable, selected = false, onchange=()=>{}})=>{
    return(
        <lable className='flex gap-3 items-center cursor-pointer mt-2 text-sm '>
            <input type="radio" name='sortOptions' checked={selected} onchange={()=>onchange(lable)} />
            <span className='font-light select-none'>{lable}</span>
        </lable>
    )
}
    ...
    px-4 md:px-16 lg:px-24 xl:px-32'>
        <div>
            <div className='flex flex-col items-start text-left'>
                ...
                <p className='text-sm md:text-base text-gray-500/90 mt-2 max-w-174'>Taker advantage of our limited-time offers and special packages to enhance your stay and create unforgotable memories.
                </p>
            </div>
            {roomsDummyData.map((room)=>(
                <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-b-gray-400 last:pb-[30px] last:border-0'>
                    <img onClick={()=>{navigate(`/rooms/${room._id}`); scrollTo(0,0)}} src={room.images[0]}  alt="hotel-image" title='VIe room Details' className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer'/>
        ...
        {/*filters*/}
        <div className='bg-white w-80 border border-gray-300 text-grayy-600 max-lg:mb-8 min-lg:mt-16'> 
